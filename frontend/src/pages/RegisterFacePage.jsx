...
export default function RegisterFacePage() {
  ...
  const handleVerificationSubmit = async (e) => {
    ...
    try {
      ...
      if (res.data.success) {
        ...
        // Don't reset results - allow continuing to capture new images
      }
    } catch (err) {
  ...
  const isFirstTimeRegistration = !faceData?.faceRegistered;
  const shouldShowCamera = isFirstTimeRegistration || (canUpdateFace && !showVerification);
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-6">
      ...
      {/* Camera - Show for first time registration or after verification for updates */}
      {shouldShowCamera && (
        <div className="bg-white rounded-2xl shadow-sm border border-slate-100 p-6">
          <WebcamCapture
            onCapture={handleCapture}
            loading={loading}
            label={isFirstTimeRegistration ? "Register This Photo" : "Update Face Image"}
          />
        </div>
      )}
